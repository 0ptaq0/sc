    ...
    class Admin
    {
        ...
        public function navigation()
        {
            return [
                ...
                $this->core->lang['settings']['translation']    => 'translation',
        ...
        public function saveGeneral()
        {
            ...
            else
            {
                ...
                if($this->core->getSettings('settings', 'autodetectlang'))
        ...
        {
            if(!isset($_GET['lang']))
                $_GET['lang'] = $this->core->getSettings('settings', 'lang_site');

            if(!isset($_GET['source']))
                $_GET['source'] = 0;

            if(isset($_POST['save'], $_POST[$_GET['source']]))
            {
                ...
                else
                {
                    ...
                    if(isset($toSave['front']))
                    {
                        ...
                        if(file_put_contents('../inc/modules/'.$_GET['source'].'/lang/'.$_GET['lang'].'.ini', $output))
                            $this->core->setNotify('success', $this->core->lang['settings']['save_file_success']);
                        else
                            $this->core->setNotify('failure', $this->core->lang['settings']['save_file_failure']);
                    }
                    ...
                    if(isset($toSave['admin']))
                    {
                        ...
                        if(file_put_contents('../inc/modules/'.$_GET['source'].'/lang/admin/'.$_GET['lang'].'.ini', $output))
                            $this->core->setNotify('success', $this->core->lang['settings']['save_file_success']);
                        else
                            $this->core->setNotify('failure', $this->core->lang['settings']['save_file_failure']);
                    }
            ...
            $settings['langs'] = $this->_getLanguagesArray($_GET['lang']);
            ...
            $this->core->tpl->set('translation', $translation);
            $this->core->tpl->set('translations', $translations);
            $this->core->tpl->set('module', $_GET['source']);
            $this->core->tpl->set('settings', $settings);

            //unset($translations, $settings);

            return $this->core->tpl->draw(MODULES.'/settings/view/admin/translation.html');
        }
		...
		* @return array
		*/
		private function _getLanguagesArray($selected = null)
		{
			$langs = glob('../inc/lang/*', GLOB_ONLYDIR);
